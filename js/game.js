...
var game = {
  points: 0, //points
  clickers: 0, //autoclicker amount
  factories: 0, //factory amount
  portals: 0 //portal amount
};
...
game.autoUnlocked = 0 // checks if automation is unlocked
game.haveUpg2 = 0 // checks if have upg 2
...
function upgrades() {
  ...
  Array.from(c("buildingLocked")).forEach(element => element.style.display = "none");
...
function loop() { // don't change this stuff unless you know what you're doing
  ...
  if(game.cost1 > game.points){$("clickerButton").className = "buildingLocked";}else{$("clickerButton").className = "buildingButton";}
  if(game.cost2 > game.points){$("factoryButton").className = "buildingLocked";}else{$("factoryButton").className = "buildingButton";}
  ...
  $("rtu33").innerHTML = "Generators gain a multiplier based on y's <br> Cost: x Reset Points <br> Currently: " + Math.round(yMult*100)/100 + "x";
  if(game.ret != 3){
  ...
  if(game.resetPoints >= 30 && !game.autoUnlocked){
    game.autoUnlocked = 1
    alert("You just unlocked the Automation tab!")
  }
  if (game.autoUnlocked) showElement("automation");
  else hideElement("automation");
  if(game.ab1 === 1){
    buyClicker();
    buyFactory();
    buyPortal();
...
function buyClicker() { // buy autoclicker function
  if(game.points >= game.cost1){
    ...
    game.clickers += 1;
    game.cost1 = Math.floor(25 * Math.pow(game.scaling, game.clickers))
    convertNumber();
    $("clickers").innerHTML = "Autoclickers: " + game.clickers;
    $("clickerButton").innerHTML = "Buy for " + game.cost1 + " points";
  }
...
function buyFactory() { // buy point factory function
  if(game.points >= game.cost2){
    ...
    game.factories += 1;
    game.cost2 = Math.floor(200 * Math.pow(game.scaling, game.factories))
    convertNumber();
    $("factories").innerHTML = "Point Factories: " + game.factories;
    $("factoryButton").innerHTML = "Buy for " + game.cost2 + " points";
  }
...
function buyPortal() { // buy point portal function
  if(game.points >= game.cost3){
    ...
    game.portals += 1;
    game.cost3 = Math.floor(15000 * Math.pow(game.scaling, game.portals))
    convertNumber();
    $("portals").innerHTML = "Point Portals: " + game.portals;
    $("portalButton").innerHTML = "Buy for " + game.cost3 + " points";
  }
...
// max stuff
function affordGeometricSeries(res, start, mult, have) {
  let realStart = start * Math.pow(mult, have)
  return Math.floor(Math.log10(res / realStart * (mult - 1) + 1) / Math.log10(mult))
}

function maxGen(costBase, have, loc, genNum) {
  let canBuy = affordGeometricSeries(game.points, costBase, game.scaling, have)
  let multThing = (Math.pow(game.scaling, 0 - (canBuy - 1)) * (Math.pow(game.scaling, canBuy - 1) - 1) / (game.scaling - 1)) + 1
  let cost = Math.floor(costBase * Math.pow(game.scaling, have + canBuy - 1) * multThing)
  
  if (canBuy >= 1) {
    game.points = game.points - cost
    game[loc] = game[loc] + canBuy
    game[`cost${genNum}`] = Math.floor(costBase * Math.pow(game.scaling, game[loc]))
  }
}

function maxAll() {
  maxGen(15000, game.portals, "portals", 3)
  maxGen(200, game.factories, "factories", 2);
  maxGen(25, game.clickers, "clickers", 1);
...
function buyMult() { // buy generator multiplier function
  if(game.x >= game.multCost){
    ...
    $("multText").innerHTML = "Current generator multiplier: " + Math.round(game.genMult) + "x";
    $("multButton").innerHTML = "Upgrade mutliplier for " + game.multCost + "x";
    if(game.multCost >= 300){
      game.multCost = 1e100; // just to make sure
      $("multText").innerHTML = "Current generator multiplier: " + Math.round(game.genMult) + "x";
      $("multButton").innerHTML = "Generator multiplier has been maxed out";
      $("multButton").className = "multButton2";
    }
  ...
  switch(id) {
    ...
    case 34:
      if(game.resetPoints >= 310 && game.rtu34 === 0){
          ...
          $("resetPoints").innerHTML = "Reset Points: " + game.resetPoints;
          showElement("subtab3");
          alert("WARNING: Do not use this mechanic! This mechanic is VERY buggy. It's the end of the road from here.");
          alert("But don't worry. This game is getting updated at certain times. Hope you had fun! Expect new content in the future!");
      }
    break;
    case 41:
...
function changeTheme(theme) { // theme change function
  switch(theme) {
    ...
    break;
    case 'nocss':
      $("style").href = "stylesheets/NewUI.css";
      $("theme").innerHTML = "Switch Theme (Currently New UI)"
      $("theme").onclick = () => changeTheme('newui');
    break;
    case 'newui':
